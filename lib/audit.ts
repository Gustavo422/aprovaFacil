...
export class AuditLogger {
   ...
   */
  async logCreate(userId: string, tableName: string, recordId: string, newValues: any): Promise<void> {
    await this.log({
      userId,
      action: 'CREATE',
      tableName,
   ...
   */
  async logUpdate(userId: string, tableName: string, recordId: string, oldValues: any, newValues: any): Promise<void> {
    await this.log({
      userId,
      action: 'UPDATE',
      tableName,
   ...
   */
  async logDelete(userId: string, tableName: string, recordId: string, oldValues: any): Promise<void> {
    await this.log({
      userId,
      action: 'DELETE',
      tableName,
   ...
   */
  async logAccess(userId: string, tableName: string, recordId: string): Promise<void> {
    await this.log({
      userId,
      action: 'ACCESS',
      tableName,
   ...
   */
  async logSimuladoComplete(userId: string, simuladoId: string, score: number, timeTaken: number): Promise<void> {
    await this.log({
      userId,
   ...
   */
  async logQuestaoComplete(userId: string, questaoId: string, score: number): Promise<void> {
    await this.log({
      userId,
   ...
   */
  async logProgressUpdate(userId: string, tableName: string, recordId: string, oldProgress: any, newProgress: any): Promise<void> {
    await this.log({
      userId,
   ...
   */
  async getResourceLogs(tableName: string, recordId: string, limit: number = 50): Promise<any[]> {
    try {
      const { data, error } = await this.supabase
        .from('audit_logs')
        .select('*')
   ...
   */
  async getLogsByPeriod(startDate: string, endDate: string, userId?: string): Promise<any[]> {
    try {
