...
export class PerformanceCalculator {
  ...
  async calculateSimuladosStats(userId: string): Promise<{
    ...
    const totalTime = data.reduce((acc, item) => acc + item.time_taken_minutes, 0)
  ...
  async calculateQuestoesStats(userId: string): Promise<{
    ...
    if (error) {
      console.error('Erro ao calcular estatísticas de questões:', error)
    ...
    data.forEach(item => {
      if (item.answers && Array.isArray(item.answers)) {
        ...
        correctAnswers += item.answers.filter((answer: any) => answer.correct).length
      }
    })
   ...
   */
  async calculateDisciplineStats(userId: string): Promise<DisciplinePerformance[]> {
    const { data, error } = await this.supabase
    ...
    if (error) {
      console.error('Erro ao calcular estatísticas por disciplina:', error)
      return []
    }
    ...
    return data.map(item => ({
      ...
      progressPercentage: item.total_questions > 0 ? (item.correct_answers / item.total_questions) * 100 : 0
    }))
  }
  ...
  async calculateWeeklyProgress(userId: string): Promise<{
    ...
    const studyTime = simuladosData.data?.reduce((acc, item) => acc + item.time_taken_minutes, 0) || 0
    ...
    const previousWeekAvg = previousWeekData.data && previousWeekData.data.length > 0 
      ? previousWeekData.data.reduce((acc, item) => acc + item.score, 0) / previousWeekData.data.length 
      : 0
    const currentWeekAvg = currentWeekData.data && currentWeekData.data.length > 0 
      ? currentWeekData.data.reduce((acc, item) => acc + item.score, 0) / currentWeekData.data.length 
      : 0

    const scoreImprovement = previousWeekAvg > 0 ? ((currentWeekAvg - previousWeekAvg) / previousWeekAvg) * 100 : 0
  ...
  async updateDisciplineStats(
    ...
    try {
      ...
      if (existingStats) {
        ...
        const newTotalQuestions = existingStats.total_questions + questionsAnswered
        ...
        const newStudyTime = existingStats.study_time_minutes + studyTimeMinutes
        ...
        await this.supabase
          ...
          .insert({
            ...
            average_score: questionsAnswered > 0 ? (correctAnswers / questionsAnswered) * 100 : 0,
      ...
      await this.cache.delete(userId, CacheManager.generateDisciplineStatsKey(userId, disciplina))
      await this.cache.delete(userId, CacheManager.generatePerformanceKey(userId, 'complete'))

    } catch (error) {
      console.error('Erro ao atualizar estatísticas de disciplina:', error)
    }
  }
  ...
  async updateUserStats(
    ...
    try {
      const { data: user } = await this.supabase
        .from('users')
        .select('total_questions_answered, total_correct_answers, study_time_minutes, average_score')
        .eq('id', userId)
        .single()

      if (user) {
        const newTotalQuestions = user.total_questions_answered + questionsAnswered
  ...
  async recordSimuladoCompletion(
    ...
    try {
      ...
      await this.auditLogger.logSimuladoComplete(userId, simuladoId, score, timeTaken)

      // Limpar cache
      await this.cache.delete(userId, CacheManager.generatePerformanceKey(userId, 'simulados'))
      await this.cache.delete(userId, CacheManager.generatePerformanceKey(userId, 'complete'))

    } catch (error) {
      console.error('Erro ao registrar conclusão de simulado:', error)
    }
  }
  ...
  async recordQuestaoCompletion(
    ...
    try {
      ...
      // Atualizar estatísticas
      await this.updateUserStats(userId, totalQuestions, correctAnswers, totalQuestions * 2) // 2 min por questão
      ...
      // Limpar cache
      await this.cache.delete(userId, CacheManager.generatePerformanceKey(userId, 'questoes'))
      await this.cache.delete(userId, CacheManager.generatePerformanceKey(userId, 'complete'))

    } catch (error) {
      console.error('Erro ao registrar conclusão de questões:', error)
    }
  }
}

// Função utilitária para obter instância do calculador
export const getPerformanceCalculator = () => PerformanceCalculator.getInstance() 