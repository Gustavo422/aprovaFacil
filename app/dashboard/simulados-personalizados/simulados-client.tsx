...
interface SimuladosClientProps {
  simuladosPersonalizados: Simulado[];
...
export default function SimuladosClient({ 
  simuladosPersonalizados: initialSimulados, 
  categorias: initialCategorias 
}: SimuladosClientProps) {
  const [simulados] = useState<Simulado[]>(initialSimulados);
  const [searchTerm, setSearchTerm] = useState('');
  const [selectedCategory, setSelectedCategory] = useState('todos');
  const [loading, setLoading] = useState(false);
  const [categorias] = useState<ConcursoCategoria[]>(initialCategorias);
  ...
  const filteredSimulados = simulados.filter(simulado => {
    ...
    const matchesCategory =
      ...
      simulado.concursos?.categoria_id === selectedCategory;
    ...
    <div className="space-y-8">
      ...
      <div className="space-y-2">
        ...
        <p className="text-muted-foreground">
          Pratique com simulados-personalizados completos para testar seus conhecimentos.
        </p>
      </div>
        ...
        <div className="text-center py-12">
          ...
          <p className="text-muted-foreground">
            {searchTerm || selectedCategory !== 'todos'
              ...
              : 'Não há simulados-personalizados disponíveis no momento.'}
          </p>
        </div>
      ) : (
          ...
          {filteredSimulados.map(simulado => (
            <Card key={simulado.id} className="card-hover flex flex-col">
              <CardHeader className="pb-4">
                  ...
                  <div className="flex flex-col gap-1 items-end">
                    {simulado.concursos?.concurso_categorias && (
                      <Badge variant="secondary" className="text-xs">
                        {simulado.concursos.concurso_categorias.name}
                      </Badge>
                    )}
                ...
                {simulado.concursos && (
                  <div className="text-xs text-muted-foreground">
                    Concurso: {simulado.concursos.name}
              ...
              <CardFooter className="pt-0">
                <Link
                  href={`/dashboard/simulados-personalizados/${simulado.slug}`}
                  className="w-full"
                >
