...
export default function ApostilasClient() {
  ...
  const fetchApostilas = async () => {
    try {
      ...
      console.error('Erro ao buscar apostilas:', error instanceof Error ? error.message : String(error));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {apostilas.map((apostila: Apostila) => (
          <Card key={apostila.id} className="h-full">
            ...
            <CardContent className="space-y-4">
              ...
              <Link
                href={`/dashboard/apostila-inteligente/${apostila.slug}`}
                className="w-full"
              >
