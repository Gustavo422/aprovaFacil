...
export default function MapaAssuntosPage() {
  const [assuntosPorDisciplina, setAssuntosPorDisciplina] = useState<Record<string, Assunto[]>>({})
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "estudado":
        return <Badge variant="default" className="bg-green-500">Estudado</Badge>
      case "a_revisar":
        return <Badge variant="secondary" className="bg-yellow-500">A Revisar</Badge>
      case "nao_sei_nada":
  ...
  return (
    <div className="flex flex-col gap-4">
      <h1 className="text-3xl font-bold tracking-tight">Mapa de Assuntos</h1>
      <p className="text-muted-foreground">Acompanhe seu progresso em cada disciplina e assunto.</p>
              ...
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <CardDescription>
                      {estudados} de {totalAssuntos} assuntos estudados ({Math.round(progresso)}%)
                    </CardDescription>
                  </div>
              ...
              <CardContent>
                <div className="grid gap-3">
                  {assuntos.map((assunto) => (
                    <div key={assunto.id} className="flex items-center justify-between p-3 border rounded-lg">
                      <div className="flex items-center gap-3">
                        {getStatusIcon(assunto.status)}
                        <div>
                          <h4 className="font-medium">{assunto.tema}</h4>
                          {assunto.subtema && (
                            <p className="text-sm text-muted-foreground">{assunto.subtema}</p>
                          )}
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        ...
                        <div className="flex gap-1">
                          <Button
                            ...
                            className={assunto.status === "estudado" ? "bg-green-50" : ""}
                          >
                            ✓
                          </Button>
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => updateStatus(assunto.id, "a_revisar")}
                            className={assunto.status === "a_revisar" ? "bg-yellow-50" : ""}
                          >
                            ⏰
                          </Button>
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => updateStatus(assunto.id, "nao_sei_nada")}
                            className={assunto.status === "nao_sei_nada" ? "bg-red-50" : ""}
                          >
                            ✗
                          </Button>
                        </div>
