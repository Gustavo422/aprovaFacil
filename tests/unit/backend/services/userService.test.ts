...
class UserService extends BaseService<User> {
  ...
  async updateProfile(usuarioId: string, profileData: Partial<User>): Promise<User | null> {
    // Validate profile data
    if (profileData.email && !this.isValidEmail(profileData.email)) {
      throw new Error('Invalid email format');
    }

    return this.repository.updateProfile(usuarioId, profileData);
  }

  async activateUser(usuarioId: string): Promise<User | null> {
    return this.repository.activateUser(usuarioId);
  }

  async deactivateUser(usuarioId: string): Promise<User | null> {
    return this.repository.deactivateUser(usuarioId);
  }
...
describe('UserService', () => {
  ...
  describe('updateProfile', () => {
    it('should update user profile successfully', async () => {
      ...
      // Act
      const result = await service.updateProfile(usuarioId, profileData);
      
      // Assert
      expect(mockRepository.updateProfile).toHaveBeenCalledWith(usuarioId, profileData);
    ...
    it('should validate email format when updating profile', async () => {
      ...
      // Act & Assert
      await expect(service.updateProfile(usuarioId, profileData)).rejects.toThrow('Invalid email format');
  ...
  describe('activateUser', () => {
    it('should activate a user successfully', async () => {
      ...
      // Act
      const result = await service.activateUser(usuarioId);
      
      // Assert
      expect(mockRepository.activateUser).toHaveBeenCalledWith(usuarioId);
      expect(result).toEqual(expectedResult);
    });
  });
  ...
  describe('deactivateUser', () => {
    it('should deactivate a user successfully', async () => {
      ...
      // Act
      const result = await service.deactivateUser(usuarioId);
      
      // Assert
      expect(mockRepository.deactivateUser).toHaveBeenCalledWith(usuarioId);
      expect(result).toEqual(expectedResult);
    });
  });
