  ...
  async getRecentActivities(userId: string, limit: number = 10) {
    return withServiceErrorHandling(async () => {
      ...
      if (simuladosProgress) {
        ...
        simuladosProgress.forEach((progress: SimuladoProgress) => {
          const simulado = Array.isArray(progress.simulados) ? progress.simulados[0] : null;
          activities.push({
            id: progress.id,
            type: 'simulado',
      ...
      if (flashcardsProgress) {
        ...
        flashcardsProgress.forEach((progress: FlashcardProgress) => {
          const flashcard = Array.isArray(progress.flashcards) ? progress.flashcards[0] : null;
          const question = flashcard?.pergunta;
      ...
      if (apostilasProgress) {
        type ApostilaProgress = {
          id: string;
          updated_at: string;
          apostila_content: Array<{
            id: string;
            title: string;
          }>;
        };

        apostilasProgress.forEach((progress: ApostilaProgress) => {
          const content = Array.isArray(progress.apostila_content) ? progress.apostila_content[0] : null;
          activities.push({
            id: progress.id,
            type: 'apostila',
            title: 'Apostila',
            description: content?.title || 'Conteúdo estudado',
            time: this.formatRelativeTime(progress.updated_at),
            created_at: progress.updated_at,
          });
        });
