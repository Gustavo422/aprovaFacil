...
import { CheckCircle, HelpCircle, XCircle, RotateCcw } from "lucide-react"
...
export function Flashcard({ flashcard, onNext, onPrev, onRate, showNavigation = true }: FlashcardProps) {
  ...
  return (
    <div className="flex flex-col gap-4 max-w-2xl mx-auto">
      <Card className={`min-h-[250px] md:min-h-[300px] transition-all duration-300 ${
        isFlipped ? "bg-muted/50" : ""
      }`}>
        <CardHeader className="pb-3">
          <CardTitle className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
            <span className="text-base md:text-lg">{flashcard.disciplina || "Flashcard"}</span>
            {flashcard.tema && (
